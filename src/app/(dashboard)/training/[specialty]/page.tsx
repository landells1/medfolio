...
export default function TrainingSpecialtyPage() {
  ...
  const loadData = useCallback(async (uid: string, cancelled: { current: boolean }) => {
    ...
    try {
      ...
      // Filter to only training templates (exclude application templates)
      const trainingTemplates = fetchedTemplates.filter((t) => t.training_year !== 'Application');
      const existingItems: PortfolioItem[] = itemsRes.data ?? [];

      setAllTemplates(trainingTemplates);
      setAllItems(existingItems);
      setLoading(false);
      baseDataLoaded = true;

      const existingTemplateIds = new Set(existingItems.map((i) => i.template_id));
      const missing = trainingTemplates.filter((t) => !existingTemplateIds.has(t.id));
      ...
      console.error('[MedFolio] Training load error:', err);
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-surface-500 mt-1">
            Track your training requirements and evidence
          </p>
        </div>
                      ...
                      {categoryTemplates.map((template) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Link
                              href={`/training/${specialtyId}/evidence/${item.id}`}
                              prefetch={false}
                              className={cn(
